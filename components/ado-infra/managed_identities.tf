...
data "azurerm_user_assigned_identity" "azure-devops-mi" {
  provider            = azurerm.workload_identity
  name                = local.managed_identity_object_name
  resource_group_name = data.azurerm_resource_group.mi-rg.name
}
...
resource "azurerm_role_assignment" "mi_landing_contributor" {
  ...
  principal_id         = var.env == "dev" ? data.azurerm_user_assigned_identity.azure-devops-mi : azurerm_user_assigned_identity.azure-devops-mi[count.index].principal_id
}
